...
uses {$IFDEF WIN32} Windows, Registry, {$ELSE} WinTypes, WinProcs, {$ENDIF}
  Controls, Messages, Classes, Forms, IniFiles, Dialogs, VclUtils, RxHook, Variants, RTLConsts;
...
begin
  ...
  DisposeStr(PAnsiString(FIniFileName));
  DisposeStr(PAnsiString(FIniSection));
  FWinMinMaxInfo.Free;
  inherited Destroy;
end;
...
begin
  AssignStr(PAnsiString(FIniFileName), Value);
end;
...
begin
  AssignStr(PAnsiString(FIniSection), Value);
end;
