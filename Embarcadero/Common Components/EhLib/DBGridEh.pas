...
{ TCustomDBGridEh }
  ...
  TDBGridEhOption = (dghFixed3D, dghFrozen3D, dghFooter3D, dghData3D, dghResizeWholeRightPart,
    ...
    dghPreferIncSearch, dghRowHighlight, dghDblClickOptimizeColWidth);
...
(*  TDrawDataCellEvent = procedure (Sender: TObject; const Rect: TRect; Field: TField;
  ...
  TCustomDBGridEh = class(TCustomGrid, IInplaceEditHolderEh, IUnknown)
  private
...
begin
  ...
  DrawText(Grid.Canvas.Handle, PChar(ws), Length(ws), NewRect, uFormat);
...
type
  ...
  TInplaceFilterEditEh = class(TDBEditEh)
  protected
...
begin
  ...
  begin
    ...
    if (self.DrawMemoText = True) and (Field.DataType = ftMemo) then
...
begin
  ...
  else if (ShortCut(Key, Shift) = DBGridEhInplaceSearchKey) and (dghIncSearch in OptionsEh)
  ...
  begin
    ...
    begin
      ...
      FilterRect := Rect(FilterRect.Right, FilterRect.Top, FilterRect.Right
        + FInplaceEditorButtonWidth, FilterRect.Bottom);
...
begin
  ...
  if FTryUseMemTableInt and (FDataLink <> nil) and
    (FDataLink.DataSet <> nil) and (FooterRowCount = 0)
...
begin
  ...
  begin
    ...
    begin
      NewCol := Col;
      MoveColRow(FixedCols, NewRow, False, True);
      MoveColRow(NewCol, NewRow, False, False);
    end else
      MoveColRow(Col, NewRow, False, True);
...
type
  TCustomDBEditEhCracker = class(TCustomDBEditEh) end;
...
begin
  ...
  begin
    ...
    with TCustomDBEditEhCracker(FFilterEdit) do
    begin
//    FilterEdit.FGrid := Self;
      Visible := False;
      Parent := {Parent;//} Self;
      SetInplaceEditHolder(Self);
      BorderStyle := bsNone;
      Font := Self.Font;
      Font.Color := clBlue;
      Color := LightenColor(FixedColor, 8); //FixedColor;
...
begin
  FilterEdit.Value := Null;
  TCustomDBEditEhCracker(FilterEdit).EditButton.Visible := False;
  FilterEdit.Text := GetFilterValue(DataCol);
end;
...
var
  OldLeftCol: Integer;
begin
  if (FFilterCol <> -1) and (FilterEdit <> nil) then
    SetFilterValue(FFilterCol);
  OldLeftCol := LeftCol;
  FLockPaint := True;
  try
...
begin
//  if TInplaceFilterEditEh(Control).ListVisible then
//    Exit;
  case Key of
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        FText := S;
        FCol := 0;
        FRow := Column.Index + IndicatorOffset;
...
function TCustomDBGridEh.GetRowLinesHeight: Integer;
begin
  if dgRowLines in Options then
  begin
    Result := 1
...
function TCustomDBGridEh.GetColLinesWidth: Integer;
begin
...
procedure TCustomDBGridEh.CMHintShow(var Message: TMessage);
var
  VMess: TCMHintShow;
  VRect: TRect;
...
begin
  ...
  begin
    ...
    try
      ...
      VHint := Columns[VCol].Field.AsString; //DisplayText;
    finally
...
procedure TCustomDBGridEh.WMRButtonDown(var Message: TWMMouse);
var
  Coord: TGridCoord;
begin
...
procedure TCustomDBGridEh.WMLButtonDown(var Message: TWMMouse);
var
  Coord: TGridCoord;
begin
  Coord := MouseCoord(Message.XPos, Message.YPos);
      ...
      begin
        ...
        if FilterEdit.Visible then
          FilterEdit.Perform(WM_LBUTTONDOWN, Message.Keys,
            Longint(PointToSmallPoint(FilterEdit.ScreenToClient(ClientToScreen(SmallPointToPoint(Message.Pos))))));
        //SendCancelMode(nil);
        FDownMouseMessageTime := GetMessageTime;
        FDownMousePos := Point(Message.XPos, Message.YPos);
        //ResentMouseEvent(Message, ClientToScreen(SmallPointToPoint(Message.Pos)));
...
begin
  if MemTableSupport and FDatalink.Active then
  begin
    Result := False;
...
begin
  ...
  if FGrid.MemTableSupport then
  begin
    TopRow := FGrid.FIntMemTable.InstantReadIndexOfBookmark(TBookmark(ATop)) + FGrid.TopDataOffset;
    BottomRow := FGrid.FIntMemTable.InstantReadIndexOfBookmark(TBookmark(ABottom)) + FGrid.TopDataOffset;
  end else
  begin
...
{    OldActive := FGrid.TopRow - FGrid.TopDataOffset;
    ...
    TopRow := FGrid.FIntMemTable.InstantReadIndexOfBookmark(TBookmark(Self.TopRow));
    BottomRow := FGrid.FIntMemTable.InstantReadIndexOfBookmark(TBookmark(Self.BottomRow));
  end else
  begin
    OldActive := FGrid.DataLink.ActiveRecord;
    for i := 0 to FGrid.DataLink.RecordCount - 1 do
...
procedure TSTColumnFilterEh.Cliear;
begin
  ExpressionStr := '';
end;
...
begin
  ...
  else Result := DataField;
end;

function TSTColumnFilterEh.GetOperand1: Variant;
begin
  Result := FExpression.Operand1;
end;
