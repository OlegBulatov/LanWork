...
type
  ...
  public
    ...
    procedure Add(Username, Password, Database: string; Fixed, Sort: Boolean);
...
procedure THistoryItems.Add(Username, Password, Database: string; Fixed, Sort: Boolean);
...
begin
  ...
  if Size > 0 then
    while NonFixedCount >= Size do Self.Item(Size - 1).Free;
  // Add the new item
...
{$ENDIF}
  ...
  try
    ...
    repeat
      ...
      begin
        S := LogonDecrypt(S);
        LogonHistory.Add(S, '', '', False, False);
      end;
      inc(i);
    until S = '';
  except
...
begin
  ...
  begin
    ...
    if (not (ldConnectAs in Options)) or
       ...
       (Session.Preferences.UseOCI7) then
    begin
    ...
    repeat
      ...
      begin
        ...
        try
          ...
          begin
            LogonHistory.Add(Session.LogonUsername, Session.LogonPassword, Session.LogonDatabase, False, True);
          end;
        except
...
begin
  ...
  if LogonHistory.WithPassword then
