...
// Based on Borland's Inter-Process Communication (IPC) example
// Copyright 2000 Allround Automations
...
begin
  Handle := CreateMutex(nil, False, PChar(Name));
...
begin
  Handle := CreateEvent(nil, Manual, False, PChar(Name));
  if Handle = 0 then abort;
end;
...
begin
  try
    FName := Name;
    FSize := Size;
    Handle := CreateFileMapping($FFFFFFFF, nil, PAGE_READWRITE, 0, Size, PChar(Name));
    if Handle = 0 then abort;
