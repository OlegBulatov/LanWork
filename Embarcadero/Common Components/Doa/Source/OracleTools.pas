...
// Copyright 2000, 2001 Allround Automations
...
uses
  ...
  ExtCtrls,  StdCtrls, ComCtrls, Menus, FileCtrl, ShellAPI,
  ToolIntf, ExptIntf, Oracle, OracleExplorer, OraclePreferences, Buttons,
  ImgList;
{$ELSE}
uses
  SysUtils, Classes, QGraphics, QControls, QForms, QDialogs,
  QExtCtrls,  QStdCtrls, QComCtrls, QMenus, ShellAPI, QImgList, Qt,
  ToolIntf, ExptIntf, Oracle, OracleExplorer, OraclePreferences, QButtons;
{$ENDIF}
...
type
  ...
  TWizardForm = class(TForm)
    ...
    procedure PackageFunctionsTreeEdited(Sender: TObject; Node: TTreeNode;
      var S: String);
    ...
    procedure PackageFunctionsTreeKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
begin
  ...
  if not Result then ShowExecuteError('Documentation', S, 'Please make sure you have installed Direct Oracle Access correctly');
  {$ENDIF}
end;
...
procedure OracleToolExecute(MenuIndex: Integer);
begin
  case MenuIndex of
    1 : DoPackageWizard;
    3 : ExecuteExplorer(nil, nil);
    4 : ExecuteOracleMonitor;
    5 : ExecutePLSQLDeveloper;
    {$IFNDEF LINUX}
    7 : WinHelp(Application.Handle, PChar(DOAHelpFile), Help_Index, 0);
    {$ENDIF}
    8 : ExecuteOracleDoc('manual.doc');
   10 : DoOraclePreferences;
   11 : ShowDOAInfo(nil);
  end;
end;
...
begin
  ...
  begin
    ...
    Query.Free;
    S.Free;
    FPureSpec := RemoveSQLComment(FPackageSpecification);
  end;
...
begin
  OracleToolExecute(Sender.GetIndex + 1);
end;
