...
type
  ...
  private
    ...
    function  CreateSession(const host: PAnsiChar; port: integer): P_blpapi_Session_t;
...
//    procedure ProcessMessageElement(element: P_blpapi_Event_t; parentName: PAnsiChar; parentNo: integer);
    //
   ...
   public
    ...
    procedure OpenService(const serviceName: PAnsiChar);
    //
...
function streamWriter(const data: PAnsiChar; length: integer; stream: Pointer): integer; cdecl;
begin
  Assert(data <> nil);
...
function TBloomberg.CreateSession(const host: PAnsiChar; port: integer): P_blpapi_Session_t;
var
...
procedure TBloomberg.OpenService(const serviceName: PAnsiChar);
begin
...
begin
  try
    ...
    FormMain.Log(Self, LT_REQUEST, 'Reference request...', True);
...
begin
  try
    Assert(FBlpSession <> nil);
    FormMain.Log(Self, LT_REQUEST, 'Historical request...', True);
    // запрашиваем Bloomberg
    try
...
begin
  ...
  for i := 0 to securities.Count-1 do begin
    SecurityName := securities.Values[securities.Names[i]];
    bts := TEncoding.UTF8.GetBytes(SecurityName);
    SetLength(bts, Length(bts)+1);
    bts[Length(bts)-1] := 0;//надо добавить после всех символов ноль, иначе происходит смешение с предыдущими значениями
    Blp_AppendElement(securitiesElements, PAnsiChar(bts));
  end;
end;
...
begin
  ...
  for i := 0 to fields.Count-1 do begin
    bts := TEncoding.UTF8.GetBytes(fields[i]);
    SetLength(bts, Length(bts)+1);
    bts[Length(bts)-1] := 0;//надо добавить после всех символов ноль, иначе происходит смешение с предыдущими значениями
    Blp_AppendElement(fieldsElements, PAnsiChar(bts));
  end;
end;
...
begin
  if overrides.Count > 0 then
    ...
    for i := 0 to overrides.Count-1 do
      ...
      Blp_SetElement(overrideElement, 'fieldId', PAnsiChar(vFieldId));
      Blp_SetElement(overrideElement, 'value', PAnsiChar(vValue));
      end;
    end;
end;
...
begin
  if periodicity = '' then
    ...
    Blp_SetElement(elements, 'periodicitySelection', PAnsiChar(periodicity));
end;
...
begin
  ...
  Blp_SetElement(elements, 'startDate', PAnsiChar(DateToStr(startDate)));
  Blp_SetElement(elements, 'endDate', PAnsiChar(DateToStr(endDate)));
...
begin
  if Currency <> '' then
    Blp_SetElement(elements, 'currency', PAnsiChar(currency));
end;
...
procedure TBloomberg.ProcessMessageElement(element: P_blpapi_Event_t;
  parentName: PAnsiChar; parentNo: integer);
var
  elementName: PAnsiChar;
  valuesCount: integer;
