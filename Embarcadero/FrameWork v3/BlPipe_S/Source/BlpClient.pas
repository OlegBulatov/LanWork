...
type
  ...
  public
    //
    constructor Create(AOwner: TComponent; AAddress: string = ''; APort: string = '');
    ...
    // Common request - должен вернуть:
    //   0 и данные в Data
    //   или отрицательный код ошибки и описание ошибки в Data
    //   или L020_ID и пустой Data (для DB-запросов) - ???
    function SendRequest(var Data: string): integer; virtual;
    //
...
constructor TBlpClient.Create(AOwner: TComponent; AAddress: string = ''; APort: string = '');
begin
  inherited Create;
...
//  FClient.Host := BLP_HOST;
  if AAddress = '' then
    FClient.Address := BLP_HOST
  else
    FClient.Address := AAddress;

//  FClient.Port := BLP_PORT;
  if APort = '' then
    FClient.Port := BLP_PORT
  else
    FClient.Port := StrToInt(APort);
...
begin
  ...
  if not FClient.Active then
    ...
    FServiceReady := False;
    FClient.Open;
//ShowMessage('Connecting to: Host='+FClient.Host+', Address='+FClient.Address+', Port='+IntToStr(FClient.Port)+', Service='+FClient.Service);
    Log(LT_WAIT_EVENT, 'Waiting for service 3 min...');
    Result := WaitFor(@FServiceReady, 3*60*1000);
