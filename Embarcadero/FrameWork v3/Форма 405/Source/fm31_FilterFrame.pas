...
function Tfm31_Filter.GetSearchText: string;
begin
  Result := edSearchText.Text;
end;

procedure Tfm31_Filter.SetSearchText(const Value: string);
begin
  edSearchText.Text := Value;
end;

procedure Tfm31_Filter.actSearchExecute(Sender: TObject);
begin
  // начинаем поиск
  StartSearch;
end;

procedure Tfm31_Filter.StartSearch;
var
  VIndex: Integer;
  VColumn: TColumn;
...
procedure Tfm31_Filter.actSelectExecute(Sender: TObject);
var
  Position: TPoint;
  Control: TControl;
begin
...
procedure Tfm31_Filter.OnppmSearchFrameOnClick(Sender: TObject);
begin
...
procedure Tfm31_Filter.SetKeyFieldName(const Value: string);
var
  I: Integer;
  VColumn: TColumn;
  VMenuItem: TMenuItem;
...
procedure Tfm31_Filter.CheckPrm(const AValue: Pointer);
begin
  // проверка
...
procedure Tfm31_Filter.actCloseExecute(Sender: TObject);
begin
  Self.Visible := False;
end;

procedure Tfm31_Filter.edSearchTextKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #13 then
  begin
...
constructor Tfm31_Filter.Create(AOwner: TComponent);
begin
  inherited Create(AOwner); 
  Self.Visible := False;
end;

destructor Tfm31_Filter.Destroy;
begin
...
procedure Tfm31_Filter.ShowOrHide;
begin
...
procedure Tfm31_Filter.SetDBGrid(const Value: TxxxDBGrid);
var
  I: Integer;
  VColumn: TColumn;
  VMenuItem: TMenuItem;
...
function Tfm31_Filter.ColumnVisible(const AColumn: TColumn): Boolean;
begin
  Result := AColumn.Visible;
end;

function Tfm31_Filter.GetDBGrid: TxxxDBGrid;
begin
  Result := FDBGrid;
end;

procedure Tfm31_Filter.actShowHideExecute(Sender: TObject);
begin
  ShowOrHide;
end;

procedure Tfm31_Filter.SetDBLookupCB(const Value: TDBLookupComboBox);
begin
  // проверка
  CheckPrm(Value);
  FDBLookupCB :=Value;
...
function Tfm31_Filter.GetDBLookUpCB: TDBLookupComboBox;
begin
  Result := FDBlookupCB;
end;

procedure Tfm31_Filter.Hide;
begin
  // прячем фильтр
  Visible := False;
  Enabled := False;
