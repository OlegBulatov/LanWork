...
Direct Oracle Access control v4.1 Delphi2009 Show Oracle.pas Source code
Return Download Direct Oracle Access control v4.1 Delphi2009: download Oracle.pas Source code - Download Direct Oracle Access control v4.1 Delphi2009 Source code - Type:.pas 


// Direct Oracle Access - Main unit
// Copyright 1997 - 2008 Allround Automations
...
//
//  Version 4.1...
//    - Improved TOracleScript.BuildCommands
//    - Improved ClearQBE
//    - Delphi 2009 changes:
//      > TOracleSession.Preferences.ConvertUTF is now fixed to cuUTF8ToUTF16
//      > TOracleSession.BytesPerCharacter is now bcAutoDetect by default
//      > When connected to database with a Unicode database charactset,
//        variables are declared as Unicode by default
...
type
  ...
  private
    ...
    function  GetConvertUTF: TConvertUTFOption;
  ...
  published
    ...
    property ConvertUTF: TConvertUTFOption read GetConvertUTF write SetConvertUTF default cuNone;
  ...
  published
    ...
    property BytesPerCharacter: TBytesPerCharacterOption read FBytesPerCharacter write SetBytesPerCharacter default {$IFNDEF CompilerVersion2009} bc1Byte {$ELSE} bcAutoDetect {$ENDIF};
  ...
  private
    ...
    ParsedSQL: AnsiString;                // last parsed SQL statement
...
var // Always use the default character set for variables
  ForceDefaultVariableCharSet: Boolean = {$IFNDEF CompilerVersion2009} False {$ELSE} True {$ENDIF};
...
function OracleWideString(const s: AnsiString): WideString;
function OracleByteString(const s: WideString): AnsiString;
...
type TCSet = set of AnsiChar;
function  WideCharIn(WC: WideChar; const S: TCSet): Boolean;
function  CharIn(C: Char; const S: TCSet): Boolean;
...
const // Version & license information
  ...
  SerialNumber: AnsiString = '18253.6223';
  Licenses: AnsiString     = '1         ';
  DOAType: AnsiString      = 'OBJ       ';
...
begin
  Result := 3;
  {$IFDEF CompilerVersion4} inc(Result); {$ENDIF}            // 4
  {$IFDEF CompilerVersion5} inc(Result); {$ENDIF}            // 5
  {$IFDEF CompilerVersion6} inc(Result); {$ENDIF}            // 6
  {$IFDEF CompilerVersion7} inc(Result); {$ENDIF}            // 7
  {$IFDEF CompilerVersion2005} inc(Result, 1998); {$ENDIF}   // 2005
  {$IFDEF CompilerVersion2006} inc(Result); {$ENDIF}         // 2006
  {$IFDEF CompilerVersion2009} inc(Result, 3); {$ENDIF}      // 2009
  {$IFDEF CompilerVersion2010} inc(Result); {$ENDIF}
  {$IFDEF CompilerVersion2011} inc(Result); {$ENDIF}
  {$IFDEF CompilerVersion2012} inc(Result); {$ENDIF}
end;
...
begin
  ...
  {$ELSE}
    ...
    S := S + 'Serialnumber ' + Trim(String(SerialNumber));
    S := S + '  (' + Trim(String(Licenses)) + ' User license)' + #13#10;
  {$ENDIF}
...
begin
  ...
  begin
    case Mode of
      ...
      'V' : begin
              if (s[i] <= #255) and not (AnsiChar(s[i]) in ['a'..'z', 'A'..'Z', '0'..'9', '_', '#', '$', #128..#255]) then
              begin
...
function WideCharIn(WC: WideChar; const S: TCSet): Boolean;
begin
  Result := ((Ord(WC) <= $FF) and (AnsiChar(WC) in S)) or
...
begin
  ...
  Result := ((Ord(C) <= $FF) and (AnsiChar(C) in S)) or
            ((Ord(C) > $FF)  and (#$FF in S));
  {$ELSE}
...
function OracleWideString(const s: AnsiString): WideString;
begin
  if UTF8Client then Result := UTF8Decode(s) else Result := WideString(s);
end;
 
// Return a string as ansistring, based on the client character set
function OracleByteString(const s: WideString): AnsiString;
begin
  if UTF8Client then Result := UTF8Encode(s) else Result := AnsiString(s);
end;
                  ...
                  begin
                    ...
                    begin
                      if ASession.Preferences.ConvertUTF = cuUTF8ToUTF16 then
                        Result := AddCRW(UTF8Decode(PAnsiChar(buf)), ASession)
                      else
                        Result := AddCR(PAnsiChar(buf), ASession);
                    end else
                      Result := AddCR(PAnsiChar(buf), ASession);
                  end else begin
...
var Msg: array[0..AbsMaxVarchar] of AnsiChar;
begin
  Msg := '';
...
var
  sTZ: AnsiString;
begin
  sTZ := AnsiString(Format('%.2d:%.2d', [ATZHour, ATZMinute]));
  OCICall(OCIDateTimeConstruct(Session.envhp, errhp, timestamphp, AYear, AMonth,
    ADay, AHour, AMinute, ASecond, ANanoSeconds, PAnsiChar(sTZ), Length(sTZ)));
  FIsNull := False;
  Modified;
end;
...
var TS: array[0..150] of AnsiChar;
    TSL: ub4;
begin
  if IsNull then
    Result := ''
...
begin
  if Value = '' then
    ...
    OCICall(OCIDateTimeFromText(Session.envhp, errhp, PAnsiChar(AnsiValue), Length(AnsiValue), nil, 0, nil, 0, timestamphp));
    FIsNull := False;
    Modified;
  end;
end;
...
function TSessionPreferences.GetConvertUTF: TConvertUTFOption;
begin
  {$IFDEF CompilerVersion2009}
  Result := cuUTF8ToUTF16;
  {$ELSE}
  Result := FConvertUTF;
  {$ENDIF}
end;
...
begin
  ...
  FExternalLDA := nil;
  FExternalSVC := nil;
  FBytesPerCharacter := {$IFNDEF CompilerVersion2009} bc1Byte {$ELSE} bcAutoDetect {$ENDIF};
...
begin
  ...
  ServerAttached := False;
  OCICall(OCIServerAttach(srvhp, errhp, PAnsiChar(AnsiString(FDatabase)), Length(AnsiString(FDatabase)), OCI_DEFAULT));
...
var s: AnsiString;
    ...
    Err: EOracleError;
    Version: array[0..500] of AnsiChar;
    LogonMode: Integer;
    tmpenv: OCIEnv;
    tmperr: OCIError;
...
begin
  ...
  begin
    ss := Logonusername;
    if Logondatabase <> '' then ss := ss + '@' + LogonDatabase;
    SendStartActivity('Session.LogOn as ' + ss, MonitorParameters);
  end;
  MonErr := '';
        ...
        try
          ...
          begin
            ...
            if (FUsername = '') and (FPassword = '') then
              ...
              OCICall(OCIAttrSet(authp, OCI_HTYPE_SESSION, PAnsiChar(AnsiString(FUsername)), Length(AnsiString(FUsername)), OCI_ATTR_USERNAME, errhp));
              OCICall(OCIAttrSet(authp, OCI_HTYPE_SESSION, PAnsiChar(AnsiString(FPassword)), Length(AnsiString(FPassword)), OCI_ATTR_PASSWORD, errhp));
            ...
            begin
              ...
              Ver := AnsiUpperCase(String(Version));
              UseOCI80 := (Pos('ORACLE8', Ver) > 0) or
                          (Pos('ORACLE9', Ver) > 0) or
                          (Pos('RELEASE 1', Ver) > 0);
            ...
            if @olog = nil then
              orlon(LDA, HDA, PAnsiChar(s), -1, nil, -1, 0)
            else
              olog(LDA, HDA, PAnsiChar(s), -1, nil, -1, nil, -1, 0);
          end;
        finally
          RestoreCursor;
        end;
...
begin
  ...
  begin
    ...
    OCICall(OCIPasswordChange(svchp, errhp, PAnsiChar(AnsiString(FUsername)), Length(AnsiString(FUsername)),
                              PAnsiChar(AnsiString(FPassword)), Length(AnsiString(FPassword)),
                              PAnsiChar(AnsiString(NewPassword)), Length(AnsiString(NewPassword)), OCI_DEFAULT));
  end else begin
...
begin
  ExtProcCheck;
  OCIExtProcRaiseExcpWithMsg(ExtProcContext, ErrorNumber, PAnsiChar(AnsiString(ErrorMessage)), Length(AnsiString(ErrorMessage)));
end;
...
var Msg: array[0..2000] of AnsiChar;
    EmptyLDA: PLDA;
begin
  Acquire(True);
  if UseOCI80 then
...
begin
  ...
  MTSCall(OraMTSSvcGet(PAnsiChar(AnsiString(FUsername)), PAnsiChar(AnsiString(FPassword)), PAnsiChar(AnsiString(FDatabase)),
...
begin
  MTSInitialize;
  MTSCall(OraMTSSvcEnlistEx(svchp, errhp, ATransaction, ORAMTS_ENFLG_DEFAULT, PAnsiChar(AnsiString(FDatabase))),
          'OraMTSSvcEnlistEx');
  FMTSEnlisted := True;
...
begin
  MTSInitialize;
  MTSCall(OraMTSSvcEnlistEx(svchp, errhp, ATransaction, ORAMTS_ENFLG_DETCHTX, PAnsiChar(AnsiString(FDatabase))),
          'OraMTSSvcEnlistEx');
  FMTSEnlisted := False;
end;
...
begin
  MTSInitialize;
  MTSCall(OraMTSSvcEnlistEx(svchp, errhp, ATransaction, ORAMTS_ENFLG_RESUMTX, PAnsiChar(AnsiString(FDatabase))),
          'OraMTSSvcEnlistEx');
  FMTSEnlisted := True;
  FMTSTransaction := ATransaction;
end;
...
begin
  MTSInitialize;
  MTSCall(OraMTSSvcEnlistEx(svchp, errhp, nil, ORAMTS_ENFLG_DEFAULT, PAnsiChar(AnsiString(FDatabase))),
          'OraMTSSvcEnlistEx');
  FMTSEnlisted := False;
  FMTSTransaction := nil;
end;
    ...
    begin
      ...
      begin
        ...
        if VarIsNull(vv) or VarIsEmpty(vv) then
          ...
          {$IFNDEF CompilerVersion2009}
          if VarIsWideString(vv) then
            ss := UTF8Encode(vv)
          else
          {$ENDIF}
            ss := vv;
    ...
    begin
      ...
      begin
        if (bindhp <> nil) and (bindbuf = lbuf) then
          ...
          bindbuf := lbuf;
          OCICall(OCIBindByName(stmthp, bindhp, errhp, PAnsiChar(AnsiString(Name)), -1, lbuf,
        ...
        obndra(CDA, PAnsiChar(AnsiString(Name)), -1, lbuf^, lBufSize, lBufType, -1, indp,
  ...
  repeat // Build the FieldList
    ...
    begin
      ...
      begin
        cbuf[cbufl] := #0;
        {$IFDEF CompilerVersion2009}
        if UTF8Client then name := UTF8Decode(cbuf) else name := String(cbuf);
        {$ELSE}
        name := String(cbuf);
        {$ENDIF}
        case dbtype of
...
begin
  vt := VarType(V);
  Result := (vt = varString) or VarIsWideString(V);
end;
...
begin
  {$IFNDEF CompilerVersion2009}
  if UTF8Client then
    Result := UTF8Decode(AnsiString(Fieldname(FieldId)))
  else
  {$ENDIF}
...
begin
  if not UTF8Client then
    ...
    begin
      {$IFDEF CompilerVersion2009}
      if FieldIdString = WideUpperCase(Name) then
      {$ELSE}
      if FieldIdString = WideUpperCase(UTF8Decode(AnsiString(Name))) then
      {$ENDIF}
      begin
        Result := i;
        Exit;
  ...
  if FieldIsNull(FieldId) then
    ...
    begin
      if Session.Preferences.ConvertUTF = cuUTF8toUTF16 then
        ...
        Result := String(UTF8Encode(FieldAsWideString(FieldId)))
    end else begin
      Failed := False;
      ...
      case FieldType(FieldId) of
         otString: begin
                     if FD.IsUTF8 and (Session.Preferences.ConvertUTF = cuUTF8toUTF16) then
                       Result := UTF8Decode(AddCR(PAnsiChar(Buffer), Session))
                     else
                       Result := String(AddCR(PAnsiChar(Buffer), Session));
                   end;
  ...
  if FieldIsNull(FieldId) then
    ...
    begin
      ...
      if not FD.IsUTF8 then
        ...
        begin
          Result := UTF8Decode(AnsiString(FieldAsString(FieldId)))
        end else
          Result := AddCRW(UTF8Decode(PAnsiChar(Buffer)), Session);
      end;
    end;
  end;
end;
...
begin
  if FieldIsNull(FieldId) then
    ...
    Buffer := FieldData(FieldId);
    case FieldType(FieldId) of
       otString: Result := StrToInt(String(PAnsiChar(Buffer)));
...
begin
  if FieldIsNull(FieldId) then
    ...
    Buffer := FieldData(FieldId);
    case FieldType(FieldId) of
       otString: Result := StrToFloat(String(PAnsiChar(Buffer)));
      otInteger: Result := PInteger(Buffer)^;
...
begin
  if FieldIsNull(FieldId) then
    ...
    Buffer := FieldData(FieldId);
    case FieldType(FieldId) of
       otString: Result := StrToDate(String(PAnsiChar(Buffer)));
      otInteger: Result := PInteger(Buffer)^;
        otFloat: Result := PDouble(Buffer)^;
...
begin
  ...
  begin
    ...
    case VarType(v) and VarTypeMask of
      ...
      varString,
      {$IFDEF CompilerVersion2009} varUString, {$ENDIF}
      varOleStr: Parameter.DataType := otString;
...
begin
  ...
  begin
    ...
    for i := 1 to Length(S) do
    begin
      if not CharIn(S[i], [#13, #10, #9, ' ']) then
      begin
        W := W + S[i];
      end else begin
        if W <> '' then
...
begin
  ...
  begin
    case Mode of
      ...
      'V' : begin
              if not ((s[i] = '&') or CharIn(s[i], ['a'..'z', 'A'..'Z', '0'..'9', '_', '#', '$', #128..#255])) then
              begin
...
begin
  ...
  for i := 1 to Length(S) do
  begin
    if CharIn(S[i], Identifiers) then
    begin
      W := W + UpperCase(S[i]);
    end else begin
      if W <> '' then
...
var i, StartIndex: Integer;
 ...
 function NotInString(const S: string): Boolean;
 var i: integer;
 begin
   Result := True;
 ...
 begin
   Result := False;
   SS := TrimRight(RemoveSQLComment(Line, False));
   if SS = '' then Exit;
   Result := ((SS = '/') or (SS = #13#10 + '/')) and NotInString(SS);
   if (not Result) and (C = ';') then Result := (SS[Length(SS)] = ';');
 end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Ver >= 2 then
      begin
        SetLength(ws, Temp div 2);
        ReadBuffer(PWideChar(ws)^, Temp);
      end else begin
        SetLength(s, Temp);
        ReadBuffer(PAnsiChar(s)^, Temp);
        ws := WideString(s);
      end;
      with Add(ws) do
      begin
...
begin
  ...
  begin
    ...
    begin
      ...
      Temp := Length(ws) * 2;
      WriteBuffer(Temp, SizeOf(Temp));
      WriteBuffer(PWideChar(ws)^, Temp);
      Temp := Ord(DataType);
      WriteBuffer(Temp, SizeOf(Temp));
      WriteBuffer(DataSize, SizeOf(DataSize));
      Temp := Length(DateFormat);
      WriteBuffer(Temp, SizeOf(Temp));
      WriteBuffer(PAnsiChar(AnsiString(DateFormat))^, Temp);
    end;
  end;
end;
...
var lub1: ub1;
    ...
    NamePart, SchemaPart: AnsiString;
begin
  FreeHandles;
  ...
  if i > 0 then
  begin
    NamePart   := AnsiString(Copy(TableName, i + 1, Length(TableName)));
    SchemaPart := AnsiString(Copy(TableName, 1, i - 1));
  end else begin
    NamePart   := AnsiString(TableName);
    SchemaPart := '';
  end;
  OCICall(OCIAttrSet(dpctx, OCI_HTYPE_DIRPATH_CTX, PAnsiChar(NamePart), Length(NamePart), OCI_ATTR_NAME, errhp));
  if OCI90 or (SchemaPart <> '') then
    OCICall(OCIAttrSet(dpctx, OCI_HTYPE_DIRPATH_CTX, PAnsiChar(SchemaPart), Length(SchemaPart), OCI_ATTR_SCHEMA_NAME, errhp));
  // OCI_ATTR_SUB_NAME
  if PartitionName <> '' then
    OCICall(OCIAttrSet(dpctx, OCI_HTYPE_DIRPATH_CTX, PAnsiChar(AnsiString(PartitionName)), Length(AnsiString(PartitionName)), OCI_ATTR_SUB_NAME, errhp));
  // OCI_ATTR_NUM_COLS
  lub2 := Columns.Count;
  ...
  // OCI_ATTR_DATEFORMAT
  if DateFormat <> '' then
    OCICall(OCIAttrSet(dpctx, OCI_HTYPE_DIRPATH_CTX, PAnsiChar(AnsiString(DateFormat)), Length(AnsiString(DateFormat)), OCI_ATTR_DATEFORMAT, errhp));
  ...
  begin
    ...
    Col := Columns[i];
    // OCI_ATTR_NAME
    OCICall(OCIAttrSet(paramhp, OCI_DTYPE_PARAM, PAnsiChar(AnsiString(Col.Name)), Length(AnsiString(Col.Name)), OCI_ATTR_NAME, errhp));
    // OCI_ATTR_DATA_SIZE
    lub4 := Col.DataSize;
    ...
    if Col.DateFormat <> '' then
      OCICall(OCIAttrSet(paramhp, OCI_DTYPE_PARAM, PAnsiChar(AnsiString(Col.DateFormat)), Length(AnsiString(Col.DateFormat)), OCI_ATTR_DATEFORMAT, errhp));
...
begin
  ...
  OCICall(OCISessionPoolCreate(envhp, errhp, spoolhp,
                               pn, pnlen,
                               PAnsiChar(AnsiString(LogonDatabase)), Length(AnsiString(LogonDatabase)),
                               0, $7FFFFFFF, 1,
                               PAnsiChar(AnsiString(LogonUsername)), Length(AnsiString(LogonUsername)),
                               PAnsiChar(AnsiString(LogonPassword)), Length(AnsiString(LogonPassword)),
                               Mode));
...
var Found: LongBool;
    RetTagInfo: PAnsiChar;
    RetTagInfoLen: ub4;
begin
  // Set the authentication information
  OCICall(OCIAttrSet(authinfop, OCI_HTYPE_AUTHINFO,
                     PAnsiChar(AnsiString(ASession.LogonUsername)), Length(AnsiString(ASession.LogonUsername)),
                     OCI_ATTR_USERNAME, errhp));
  OCICall(OCIAttrSet(authinfop, OCI_HTYPE_AUTHINFO,
                     PAnsiChar(AnsiString(ASession.LogonPassword)), Length(AnsiString(ASession.LogonPassword)),
                     OCI_ATTR_PASSWORD, errhp));
  // Get a session
  OCICall(OCISessionGet(envhp, errhp, Result, authinfop,
                        PAnsiChar(AnsiString(FPoolName)), Length(AnsiString(FPoolName)),
...
type Targnm = array[0..10000, 0..29] of AnsiChar;
var Par:TProcedureParameter;
    i: Integer;
    ProcName, PkgName: AnsiString;
    // OCI7
...
begin
  ...
  begin
    ...
    if UTF8Client then
      ...
      ProcName := AnsiString(ObjectOwner + '.' + ObjectName);
    PkgName := '';
    if SubName <> '' then
    begin
      PkgName := ProcName;
      ...
      if Overloadings = 0 then
        raise Exception.Create('Procedure ' + String(ProcName) + ' within package ' + ObjectName + ' does not exist');
      ...
      OCICall(OCIDescribeAny(Session.svchp, Session.errhp,
                             PAnsiChar(ProcName), Length(ProcName),
    ...
    ProcName := AnsiString(ObjectOwner + '.' + ObjectName);
    if SubName <> '' then ProcName := ProcName + '.' + AnsiString(SubName);
    UsedSize := 1000;
    arrsiz := UsedSize;
    ...
    odessp(ASession.LDA, PAnsiChar(ProcName), -1, nil, 0, nil, 0, ovrld^, pos^, level^,
...
begin
  ...
  begin
    ...
    OCICall(OCIDescribeAny(Session.svchp, Session.errhp,
                           PAnsiChar(AnsiString(PkgName)), Length(AnsiString(PkgName)),
                           OCI_OTYPE_NAME, 0, OCI_PTYPE_PKG, Desc));
    // Get the parameter handle for the package
    OCICall(OCIAttrGet(Desc, OCI_HTYPE_DESCRIBE, PkgParam, nil,
                       OCI_ATTR_PARAM, Session.errhp));
    ...
    begin
      ...
      if FindSubprogram(String(cbuf)) < 0 then
      begin
        SubProg := TPackageSubprogram.Create(Subprograms);
        SubProg.SubprogramName := String(cbuf);
        // Is it a function?
        PType := 0;
        OCICall(OCIAttrGet(ProcParam, OCI_DTYPE_PARAM, PType, nil,
                           OCI_ATTR_PTYPE, Session.errhp));
...
begin
  ...
  if ALOBType = otCLOB then
  begin
    if ForceDefaultVariableCharSet and (ASession <> nil) and (ASession.CurrentCharSetID > 0) and
      (ASession.CurrentCharSetIsUTF8 or ASession.CurrentCharSetIsUTF16) then
    FCharSetID := ocsUTF16;
  end;
...
begin
  ...
  begin
    if MonitorPresent then SendStartActivity('LOB.SetSize');
    try
      Session.ClearInTransaction;
      if IsUTF16 then NewSize := NewSize div 2;
...
var dn: array[0..30] of AnsiChar;
    fn: array[0..255] of AnsiChar;
    dnl, fnl: ub2;
    Res: AnsiString;
begin
  ...
  SetLength(Res, fnl);
  if fnl > 0 then Move(fn[0], Res[1], fnl);
  Result := string(Res);
end;
 
procedure TLOBLocator.SetFilename(const Value: string);
var dn, fn: AnsiString;
    Slash: Integer;
begin
  CheckNilLOB;
  CheckType(OCI_DTYPE_FILE, 'Filename');
  ...
  if Slash <= 0 then
  begin
    if IsNull then dn := ' ' else dn := AnsiString(Directory);
    fn := AnsiString(Value);
  end else begin
    fn := AnsiString(System.Copy(Value, Slash + 1, 255));
    dn := AnsiString(System.Copy(Value, 1, Slash - 1));
  end;
  OCICall(OCILOBFileSetName(Session.envhp, errhp, PLOB^, PAnsiChar(dn), Length(dn),
                            PAnsiChar(fn), Length(fn)));
  LOBModified;
end;
...
begin
  ...
  OCICall(OCILOBFileGetName(Session.envhp, errhp, PLOB^, dn, dnl, fn, fnl));
  SetLength(Res, dnl);
  if dnl > 0 then Move(dn[0], Res[1], dnl);
  Result := String(Res);
end;
...
begin
  CheckNilLOB;
  CheckType(OCI_DTYPE_FILE, 'Directory');
  if IsNull then fn := ' ' else fn := AnsiString(Filename);
  OCICall(OCILOBFileSetName(Session.envhp, errhp, PLOB^, PAnsiChar(AnsiString(Value)),
                            Length(AnsiString(Value)), PAnsiChar(fn), Length(fn)));
  LOBModified;
end;
...
begin
  ...
  SetLength(Res, ASize * cs);
  Seek(soFromBeginning, 0);
  rs := Read(Res[1], ASize * cs);
  SetLength(Res, rs);
  Result := String(Res);
...
begin
  if IsUTF16 then
    ...
    Seek(soFromBeginning, 0);
    if LOBType = otCLOB then
      s := AnsiString(RemoveCR(Value, Session))
    else
      s := AnsiString(Value);
    Write(s[1], Length(s));
    Trim;
...
begin
  ...
  Result := OCIDescribeAny(Session.svchp, errhp, PAnsiChar(AnsiString(TypeName)), Length(AnsiString(TypeName)),
...
begin
  ...
  if IsElement then
    ...
    cbuf[cbufl] := #0;
    {$IFDEF CompilerVersion2009}
    if UTF8Client then FName := UTF8Decode(cbuf) else FName := String(cbuf);
    {$ELSE}
     FName := String(cbuf);
    {$ENDIF}
  end;
  ...
  begin
    ...
    begin
      ...
      Owner.OCICall(OCIDescribeAny(Owner.Session.svchp, Owner.errhp,
                                   PAnsiChar(AnsiString(FTypeName)), Length(AnsiString(FTypeName)),
  ...
  if IsNull then
    ...
    case DataType of
       otString,
          otRaw: begin
                   v := Value;
                   if VarIsWideString(Value) and (Owner.Session.Preferences.ConvertUTF <> cuUTF8toUTF16) then
                     Result := String(UTF8Encode(v))
                   else
                     Result := string(v);
                 end;
...
begin
  ...
  SetLength(Res, l);
  OCICall(OCIRefToHex(Session.envhp, errhp, PRef^, PAnsiChar(Res), ub4(l)));
  SetLength(Res, l);
  Result := String(Res);
end;
...
begin
  OCICall(OCIRefFromHex(Session.envhp, errhp, Session.svchp, PAnsiChar(AnsiString(Value)),
                        Length(AnsiString(Value)), PRef^));
  RefModified;
end;
...
begin
  ...
  try
    ...
    begin
      ...
      OCICall(OCIObjectPinTable(Session.envhp, errhp, Session.svchp, PAnsiChar(AnsiString(TableOwner)),
                                Length(AnsiString(TableOwner)), PAnsiChar(AnsiString(TableName)), Length(AnsiString(TableName)),
...
var iValue: Integer;
    ...
    ABufType: sb2;
    ABufSize: sb4;
    TS: array[0..150] of AnsiChar;
    TSL: ub4;
const HexDigits: PAnsiChar = '0123456789ABCDEF';
begin
  // Is it null?
    ...
    begin
      ...
      case ABufType of
        ...
        begin
          ...
          SetLength(s, 2000);
          ABufSize := Length(s);
          OCICall(OCINumberToText(errhp, ValuePtr, PAnsiChar(Fmt1), Length(Fmt1), nil, 0, ABufSize, PAnsiChar(s)));
  ...
  if VarIsEmpty(Value) or VarIsNull(Value) then
    ...
    case AAttr.DataType of
      ...
      begin
        if AAttr.IsUTF8 then
        begin
          if VarIsWideString(Value) then
            s := UTF8Encode(RemoveCRW(Value, Session))
          else
            s := RemoveCR(AnsiString(Value), Session);
        end else if AAttr.IsUTF16 then
        begin
          if VarIsWideString(Value) then
            ws := RemoveCRW(Value, Session)
          else
            ws := RemoveCRW(UTF8Decode(AnsiString(Value)), Session);
          SwapWideString(ws, AAttr.FCharSetID);
        ...
        if s = '' then
          ...
          OCICall(OCIStringAssignText(Session.envhp, errhp, PAnsiChar(s), Length(s),
                  Pointer(ValuePtr^)));
      ...
      begin
        ...
        if (VarType(Value) = varString) or VarIsWideString(Value) then
        begin
          ...
          begin
            ...
            if s = '' then
              ...
              // Convert the string
              OCICall(OCINumberFromText(errhp, PAnsiChar(s), Length(s), PAnsiChar(Fmt), Length(Fmt), '', 0, ValuePtr));
              NullPtr[0] := OCI_IND_NOTNULL;
            end;
          end;
        end;
...
downloadOracle.pas Source code - Download Direct Oracle Access control v4.1 Delphi2009 Source code 
Related Source Codes/Software:
Delphi-like QQ form auto-hide function example - Delphi-like QQ form the auto hide feature, seen fr... 2012-02-18
Indy v9.0 version control packages downloaded with the instance - Indy v9.0.18 Internet network development control ... 2012-02-18
VCLZip Pro v3.0 Delphi control compression/decompression - VCLZip Pro is in Delphi for compression/decompress... 2012-02-18
Indy10.0.52 controls with source code download - Indy10.0.52 is the latest version of the control, ... 2012-02-18
TMS component Pack v4.2 controls with source code - TMS Component Pack V4.2 and source code version co... 2012-02-18
Delphi uses multithreaded DB recovery component interfaces automatically Query the object pool - Delphi multithreading DB component test procedures... 2012-02-18
Delphi sent point to point file example - Delphi sent point to point file example, wrote whe... 2012-02-18
XpMenu 3.1 XP menu controls - XpMenu is a XP style menu controls, which runs on ... 2012-02-18
Delphi source of keyboard hook hook procedures - A keyboard hook, DELPHI, that is, we often say tha... 2012-02-18
Delphi 16*16 dot matrix font bytecode extraction source code - Delphi wrote of small tools, from 16*16 lattice fo... 2012-02-18
IcEdit Version editor v0.9 r23 - NicEdit is a lightweight, cross-platform, inline c... 2016-08-17
Java v1.01 small game - A little game to share 1945 2016-08-17
HC Internet cafe management system v2.0 - Take xi xi Internet cafe management system is a se... 2016-08-17
BBS truts + hibernate development of source code - In struts hibernate development BBS source code! 2016-08-17
MyBlog JSP blog application - JSP blog program contains the SQL database, config... 2016-08-17
Ext JS v3.0 RC1.1 - Ext JS is a creating rich Internet applications cr... 2016-08-17
Based on the Rails phipps v0.8.5 web site management system - Software introduced Fepss (phipps) launched the fi... 2016-08-17
The books management system based on struts - Development environment:Windowsxpsp2+myEclipse5.5+... 2016-08-17
Pisces Lin JSP personnel salary system - This is using JSP to do a personnel salary system ... 2016-08-16
Liuxing ironman download system V1.0 - Iron Man download system is composed of iron man t... 2016-08-16
About Us | Contact Us | Help | English | ??? | ?? 
www.bvbcode.com ©2011  
...
begin
  ...
  qry := TOracleQuery.Create(ServiceMenuOraSession);
  try
    qry.Session := ?????-??-???-Session;
    qry.SQL.Text :=
      'declare'#13 +
      '    v_3113 EXCEPTION;'#13 +
      '    PRAGMA EXCEPTION_INIT (v_3113, -03113);'#13 +
      'begin'#13 +
      '   raise v_3113;'#13 +
      'end;'#13 +
      '';
